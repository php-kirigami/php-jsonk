...
Decisions made 2026-09-15, to be respected unless explicitly revisited.
**Nothing in this repo has been compiled or run yet** -- see "Status"
...
see decision 17 for the full write-up).** Built and tested in a throwaway
`php:8.4-cli` Docker container (this machine's WSL Ubuntu still has no
PHP installed; PHP 8.5 has no official Docker image yet -- 8.4 was close
enough in Zend API vintage for a genuine smoke test, not a stand-in for
the real 8.5 target). Eight real, distinct bugs were found and fixed by
actually compiling and running this code for the first time (decision 17
has the full list) -- proof that the "verified real API signatures"
research from earlier sessions materially de-risked this codebase but was
never a substitute for a real build. After the fixes: the extension
loads, `jsonk_encode()`/`jsonk_decode()`/`jsonk_validate()`/
`jsonk_last_error()`/`jsonk_get_last_errors()`/`JsonkException` all work
as designed, a real external `$ref` fetch (via a local test HTTP server)
correctly exercises `ext/curl` end-to-end including the `current_doc`
fix, and a round-trip correctness check
(`json_decode(json_encode($x)) === jsonk_decode(jsonk_encode($x))`)
passes on a realistic mixed-type dataset. Benchmarked against native
`json_encode()`/`json_decode()` -- decode is consistently faster
(1.76x-2.39x), encode without a schema is roughly at parity, schema
validation adds real (expected, "two passes" architecture) overhead --
see decision 17 for the full numbers and the honest read on what "ultra
fast" holds for and where it doesn't (yet). Relative `$ref` support
(decision 18, same session) was validated against a real, live,
third-party schema (`kirigami.schema.json`, three real relative refs
fetched over the network) -- not just synthetic test fixtures.
...
1. **No compile/test pass at all** (see above -- the top priority).
2. Schema compilation has no caching/reuse across calls -- every
   `jsonk_decode()`/`jsonk_encode()`/`jsonk_validate()` call with a
   `$schema` re-parses and re-compiles it from scratch. Fine for a first
   correctness pass; a real perf concern for a hot path validating the
   same schema repeatedly. A `jsonk_compile_schema(): JsonkSchema` handle
   + accepting a pre-compiled handle in place of the raw JSON string would
   be the natural fix, deferred until the uncached version is proven
   correct.
3. **Decision 6's keyword gap list is now almost entirely closed by
   decisions 15-16.** External `$ref` now works (decision 16, via
   `ext/curl`), including resolving a bare fragment nested inside a
   fetched external document against that document rather than the
   original one (decision 16's `current_doc` fix). What's left,
   genuinely: full "$id"-based relative-URI resolution (only bare
   fragments and absolute URLs are supported),
   `$dynamicRef`/`$dynamicAnchor`/`$recursiveRef` (meta-schema-authoring
   features), `$ref`-with-siblings (2019-09+ semantics -- this
   implementation treats `$ref` as exclusive of its siblings, draft-07
   style), a download-size cap on external `$ref` fetches, and
   `unevaluatedProperties`/`unevaluatedItems`'s best-effort (not yet
   conformance-tested) status. `format` and `pattern` are
   pragmatic/structural, not exhaustive-grammar -- see decision 15's own
   caveats for each.
4. `JsonSerializable` is not honored when encoding objects (only public
   properties, via `Z_OBJPROP_P()`); backed enums have no special
   handling; resources are rejected as `JSONK_ERROR_UNSUPPORTED_TYPE`
   (matching `json_encode`'s own behavior there, just without the
   `JsonSerializable` escape hatch).
5. Array/reference-based recursion is not detected during encode (only
   object-graph cycles, via `zend_object->handle` -- see decision above).
   A PHP array literally cannot self-reference by value, but a reference
   cycle (`$a['x'] = &$a;`) theoretically could still cause unbounded
   recursion caught only by the `$depth` limit, not a clean
   `JSONK_ERROR_RECURSION`.
6. `JSON_INVALID_UTF8_IGNORE`/`_SUBSTITUTE`, `JSON_HEX_*` (four flags),
   `JSON_UNESCAPED_LINE_TERMINATORS` -- still not implemented (see
   decision 19's flag-coverage answer; `JSON_PRESERVE_ZERO_FRACTION` WAS
   implemented as part of decision 17's bug fixes, no longer belongs on
   this list).
7. No `php-wasm-compiler` integration (decision 3's `static` vs `shared`
   question is unresolved, nothing has been wired into that repo).
8. Decision 13 (replace native `json_encode`/`json_decode`) -- **done, see
   decision 19**. Decision 14 (JSONPath) -- still raised, still
   explicitly deferred, not started.
9. No test suite (`run-tests.php`-style `.phpt` files, matching how a real
   PHP extension is normally tested) exists yet -- only the *intent* to
   verify via a native build has been described, no actual test has run.
